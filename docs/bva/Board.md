...
| BOARD-MOVE-010 | Black pawn is on `(5, 1)` and White pawn is on `(6, 0)`. `movePiece(6, 0, 5, 1)` is called. | White pawn captures diagonally. Starting square becomes empty and ending square contains the White pawn. | :x: |
