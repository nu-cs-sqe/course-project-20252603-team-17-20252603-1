...
| BOARD-MOVE-034 | White king at `(7, 4)`, White rook at `(7, 7)`, squares `(7, 5)` and `(7, 6)` empty, castling rights preserved. `movePiece(7, 4, 7, 6)` is called. | Kingside castling: king on `(7, 6)`, rook on `(7, 5)`; `(7, 4)` and `(7, 7)` empty. | no |
| BOARD-MOVE-035 | White king at `(7, 4)`, White rook at `(7, 0)`, squares `(7, 1)`–`(7, 3)` empty, castling rights preserved. `movePiece(7, 4, 7, 2)` is called. | Queenside castling: king on `(7, 2)`, rook on `(7, 3)`; `(7, 0)` and `(7, 4)` empty. | no |
| BOARD-MOVE-036 | White king at `(7, 4)`, White rook at `(7, 7)`, a piece occupies `(7, 5)` blocking f1. `movePiece(7, 4, 7, 6)` is called. | Move rejected; king and rook unmoved. | no |
| BOARD-MOVE-037 | White king has previously moved from `(7, 4)` and returned; White rook still on `(7, 7)`; path clear. `movePiece(7, 4, 7, 6)` is called. | Move rejected (no castling after king moved). | no |
| BOARD-MOVE-038 | White king at `(7, 4)`, White kingside rook has moved away and returned; path clear. `movePiece(7, 4, 7, 6)` is called. | Move rejected (no castling after that rook moved). | no |
| BOARD-MOVE-039 | White king at `(7, 4)`, White rook at `(7, 7)`, path clear, opponent attacks `(7, 5)` but not `(7, 4)`. `movePiece(7, 4, 7, 6)` via game rules. | Move rejected (cannot castle through check). | no |
| BOARD-MOVE-040 | White king at `(7, 4)`, White rook at `(7, 7)`, path clear, opponent attacks `(7, 6)` (g1). `movePiece(7, 4, 7, 6)` is called. | Move rejected (cannot castle into check). | no |
| BOARD-MOVE-041 | White king at `(7, 4)` in check; White rook at `(7, 7)`; path clear. `movePiece(7, 4, 7, 6)` is called. | Move rejected (cannot castle out of / while in check). | no |
| BOARD-MOVE-042 | Black king at `(0, 4)`, Black rook at `(0, 0)`, squares `(0, 1)`–`(0, 3)` empty, castling rights preserved. `movePiece(0, 4, 0, 2)` is called. | Black queenside castling: king on `(0, 2)`, rook on `(0, 3)`; `(0, 0)` and `(0, 4)` empty. | no |
| BOARD-MOVE-043 | Black king at `(0, 4)`, Black rook at `(0, 7)`, squares `(0, 5)` and `(0, 6)` empty, castling rights preserved. `movePiece(0, 4, 0, 6)` is called. | Black kingside castling: king on `(0, 6)`, rook on `(0, 5)`. | no |
