...
| GAME-SELFCHECK-006 | The White king is in check from a Black rook. White captures the checking rook with a White rook. | Move succeeds because the check is resolved. Turn changes to Black.                           | :white_check_mark: |
...
| GAME-ENPASSANT-005 | A Black pawn is not on row `4` and attempts an en passant-style diagonal move into an empty square. | The move returns `false`; the board does not change; the turn does not change. | :x: |
| GAME-ENPASSANT-006 | The previous successful move was not a two-square pawn move. A pawn attempts an en passant-style diagonal move into an empty square. | The move returns `false`; the board does not change; the turn does not change. | :x: |
| GAME-ENPASSANT-007 | The previous successful move was a two-square pawn move, but the moved pawn is not adjacent to the capturing pawn. | The move returns `false`; the board does not change; the turn does not change. | :x: |
